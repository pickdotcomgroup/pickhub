...
import { Pointer, Eye } from "lucide-react";
...
export default function AgencyBrowseDevelopersPage() {
  ...
  const [pickingTalentId, setPickingTalentId] = useState<string | null>(null);
  ...
  const handlePickDeveloper = async (talentId: string) => {
    setPickingTalentId(talentId);
    try {
      // Store picked developer in localStorage for now
      const pickedDevelopers = JSON.parse(localStorage.getItem("pickedDevelopers") ?? "[]") as string[];
      if (!pickedDevelopers.includes(talentId)) {
        pickedDevelopers.push(talentId);
        localStorage.setItem("pickedDevelopers", JSON.stringify(pickedDevelopers));
        toast.success("Developer picked successfully!", {
          position: "top-center",
          autoClose: 3000,
        ...
        // Navigate to picked developers page
        router.push("/agency/picked-developers");
      } else {
        toast.info("Developer already picked!", {
          position: "top-center",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
      ...
      console.error("Error picking developer:", error);
      setError("Failed to pick developer");
      toast.error("Failed to pick developer", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    <>
                        ...
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          {selectedTalentDetails.verification.githubUsername && (
                            <a
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-900 text-white text-sm rounded-lg hover:bg-gray-800 transition"
                            >
      ...
      <main className="min-h-screen bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Browse Developers</h1>
            <p className="text-gray-600">Discover and pick skilled professionals for your agency</p>
          </div>
                  ...
                  {talent.profile && (
                    <>
                      ...
                      <div className="flex justify-end">
                        <button
                          onClick={() => handlePickDeveloper(talent.id)}
                          disabled={pickingTalentId === talent.id}
                        ...
                        >
                          ...
                          {pickingTalentId === talent.id ? "Picking..." : "Pick"}
                        </button>
                      </div>
                    </>
                  )}
